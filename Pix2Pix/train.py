...
def main():
    ...
    if config.LOAD_MODEL:
        load_checkpoint(config.GEN_CHK, gen, opt_gen, config.LR)
        load_checkpoint(config.DISC_CHK, disc, opt_disc, config.LR)
    ...
    for epoch in range(config.EPOCHS):
        ...
        if config.SAVE_MODEL and epoch % 5 == 0:
            save_checkpoint(gen, opt_gen,filename=config.GEN_CHK)
            save_checkpoint(disc, opt_disc,filename=config.DISC_CHK)
